...
class Walker2dCrawlEnv(Walker2dEnv):
    ...
    def step(self, a):
        ...
        self.t += 1
        success = not done and self.t >= 500
    ...
    def _get_obs(self):
        ...
        qvel = self.data.qvel
        qacc = self.data.qacc
        return np.concatenate([qpos[1:], qvel, qacc]).ravel()
    ...
    def reset_model(self):
        ...
        self.set_state(
            self.init_qpos + self.np_random.uniform(low=-r, high=r, size=self.model.nq),
            self.init_qvel + self.np_random.uniform(low=-r, high=r, size=self.model.nv)
        )
